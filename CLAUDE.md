...
- **Estrutura reprovada na verificação vira PENDÊNCIA que viaja** (`MARCA_PENDENCIA` → `EstruturaProjeto.pendencias_estruturais` → `origem_regra` da 03.01): o kg continua sendo emitido (o orçamento precisa de número), mas nunca mudo. Alerta que morre no retorno é disclaimer morto.
