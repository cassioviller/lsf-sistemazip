...
- **Auditar a porta contra o v7 headless, não confiar nela** (`montarProjeto` + `resumoPorSistema` rodados via node; ver o método nos commits da BOX-003/DP-04): duas regras de nível de EDIFÍCIO estavam ausentes e silenciosas — BOX-003 (kg comprado −18%) e DP-04 (ancoragem 3,06× a mais). Hoje os acessórios batem: **30 itens em comum, 0 divergência, 0 espúrio**. Faltam 5, por decisão: 4 de **instalações** (furo de serviço, chapa de reforço, tubo-luva GLP — exigem pontos hidro/gás/elétrica, que não existem no schema: é migração) e 1 de **vento** (hold-down NBR 6123 — o v7 chumba as dimensões da 109, `vento*10,5*15,8`, e a quantidade 24 un; não generaliza e a fórmula de vento exige engenheiro estrutural, Fase 3).
- **REGRA BOX-003 é o que fecha o kg comprado** (`segmentar_box003`, aplicada em `gerar_estrutura` ANTES do corte): peça linear acima da barra comercial de 6 m não existe — vira `n = ceil(comp/6)` partes **IGUAIS** (não 6+6+resto) com emenda rastreada, que é acessório. Uma viga de 15,8 m vira 3×5,27 m e sobra 0,73 m em cada barra. **Foi a ausência dessa regra que criou a falsa "perda inexplicada de 32,4%"** que por um tempo pareceu calibração de obra pendente (R6): nestar a peça inteira como emenda enche as barras e mente ~13% para baixo no aço comprado. Depois dela, o nesting é **por sistema** (como o `resumoPorSistema` do v7, que gerou o orçamento da obra) — nestar tudo junto derrubaria o comprado ~8% supondo compra centralizada que a obra não praticou.
...
- **A R6 como definida está MORTA: a Veks não tem, e não terá, dados de obra medidos** (decisão do usuário, 2026-07-20). Toda a menção a "calibração de coeficiente `estimado` → `real` via consumo da 109.1506 / Baias Kabod" no restante deste arquivo era premissa falsa — não haverá oráculo de obra do lado da Veks. Consequência (detalhada no plano `docs/superpowers/plans/2026-07-20-calibracao-sem-obra.md`): o oráculo de custo passa a ser o **SINAPI oficial** (que o projeto já trata como `real` — a ponte grava `insumo_preco` SINAPI com `confianca='real'`). Onde o SINAPI cobre o serviço (concreto, forma, aço, placa cimentícia, drywall, preliminares/instalações/canteiro — D7), migra-se para a composição oficial → `real`. Onde o SINAPI NÃO cobre (LSF estrutural, VK-C-001 — D7), o coeficiente fica **`estimado` para sempre**, e isso não é dívida: é a postura honesta permanente do produto, comunicada pela faixa ±% na proposta (D4). Dado público secundário (CBCA, fabricante, geometria — docs/06) melhora a PROVENIÊNCIA do `estimado`, nunca o promove a `real`.
- **Seção de perfil se CALCULA, não se chumba** (`propriedades_secao`): A/Ix/Wx saem da geometria do `perfil_lsf` pelo método linear da NBR 14762 (seção como linha de espessura t). O método é validado contra os 3 valores que o v7 chumbava para o Ue250 (A=708 exato, Ix +0,08%, Wx −0,05%) — é essa validação que autoriza calcular. O v7 verificava TODA laje com a seção do Ue250, inclusive as de Ue200, superestimando MRd em **2,20×** (o Wx do Ue200#1.25 é 45% do Ue250#2.00). Verificação sempre com o perfil REALMENTE escolhido pelo escalonamento.
...
- **NENHUM projeto consegue publicar hoje, e a suíte verde esconde isso** (achado de 2026-07-20). As macroetapas **01 (Serviços preliminares), 05 (Instalações), 07 (Serviços complementares) e 08 (Gerenciamento e canteiro)** são FOLHAS sem `composicao_id`. O trigger da migração 001 recusa quantitativo em item sem composição (`"quantitativo só pode apontar para folha da EAP"`), então essas 4 nunca recebem número — e o gate R7 bloqueia a publicação para sempre. Verificado: com TODA folha mapeável preenchida, `pode_publicar=False`, `macroetapas_zeradas=['01','05','07','08']`. O caminho feliz só existe em teste porque a fixture `projeto_completo` **inventa folhas `.99` apontando para a composição da 03.01** — e o docstring dela é honesto ("ARRANJO DE TESTE, não uso do app"). Consequência: os 392 testes verdes provam os motores, não que o produto entrega proposta. **Destravar NÃO é dado da Veks — é o SINAPI real** (corrigido em 2026-07-20 depois de o usuário responder "não existe isso" à pergunta por coeficientes próprios, e com razão). Por **D7**, composição própria é só o LSF estrutural, que SINAPI não cobre; preliminares, instalações, complementares e canteiro são exatamente o escopo que SINAPI COBRE. O banco hoje tem **5 composições Veks + 2 SINAPI (96359/96114) + 11 insumos com preço**: é seed de DEMONSTRAÇÃO, não base de produção. Logo o gate R7 não está esperando planilha da Veks — está esperando o **import do SINAPI (Rota A, smoke test com arquivo da Caixa)**, que já é o item 1 do backlog docs/02 §6. Pedir coeficientes próprios para esses 4 grupos seria violar o D7 e inventar número que SINAPI publica.
...
- **Proposta .docx** (`59f7b70` + `14939f5`): `relatorios.proposta_docx` com identidade Veks, subtotais com confiança, gates e sondagem como SEÇÃO. É documento de TRABALHO e por isso NÃO passa pelo 409 — preço fechado continua saindo só por `/publicar`. python-docx 1.2.0 (MIT) registrado em docs/04; `requirements.txt` criado como registro reproduzível.
- **Cuidado com commits de fora desta sessão**: `59f7b70` veio do Replit Agent e criou a rota `.docx` **órfã** (sem link em template nenhum — só se chegava digitando a URL), com `montar` re-importado dentro da função e BDI formatado `27.79%` num documento pt-BR. Corrigido em `14939f5`/seguinte, com teste de ponta a ponta e prova de mutação. A lição: commit externo entra na árvore sem passar pelo review nem pela suíte inteira — **revisar antes de tratar como pronto**.
- **Verificação no servidor real (2026-07-20, `LSF_SECRET` + caixa 6×4 + `tests/teste.dxf`)**: upload DXF (2 paredes, aviso de classificação) ✓ · derivar (434 kg, alertas de sistema ausente como escopo) ✓ · romaneio (8 painéis, 395,9 kg líquido) ✓ · TAKEOFF 452,7 substituindo a linha ✓ · `.docx` (37 KB, content-type do Word) ✓ · **publicar com macroetapa zerada → 409** ✓ · **fundação recusada sem classe de solo** (D4.1: "fundação sem solo não é fundação com solo default") ✓.
...
1. ~~Portar o gerador de estrutura do v7~~ **FEITO** (paredes + laje + escada + cobertura + forro + BOX-003): **o critério numérico de kg está atingido** — líquido 23.673 (0,00%) e comprado 31.344 vs os 31.345 kg da obra (**−0,003%**), contra um gate de 10%, com o kg DERIVADO da planta (PARAMETRICO) e não digitado. Travado por `tests/test_aceite_estrutura_completa.py`. Falta para o aceite da fase: o item 3 abaixo (cargas por parede) e a declaração humana.
2. ~~Schema da `planta_normalizada`~~ FEITO (migração 004+008) + ~~entrada manual/assistida no app~~ **FEITO** (`app/rotas/planta.py`: CRUD do grafo nível→nó→parede→vão com nós de canto compartilhados, + botão "derivar" que roda `derivar_quantitativos` e `derivar_cargas` e mostra kg/alertas/pendências; verificado de ponta a ponta no servidor real).
3. ~~Motor de takedown de cargas por parede real~~ **FEITO** (`takedown_por_parede` + `derivar_cargas`, que grava as pendências do motor em `pendencia` p/ o gate). Desvio consciente do enunciado: kN/m NÃO vira `quantitativo` (não é folha da EAP) — é o input do pré-dim. de fundação (Fase 3), que consome o retorno. O kg PARAMETRICO da EAP já sai do `derivar_quantitativos` do gerador.

**Critério de aceite**: kg de aço da 109.1506 com desvio ≤ 10% vs v7 (referência headless: 23.673 kg líquido / 31.345 kg comprado, em `tests/fixtures/orcamento_v7_109_1506.json`) — ATINGIDO; cargas por parede validadas por cálculo próprio em `tests/test_cargas_validacao.py` (equilíbrio global ΣFv=0 na 109 + caixa 6×4 conferida à mão por NBR 6120, testes com prova de mutação). **R9 adaptado por decisão do usuário (2026-07-18)**: sem revisão externa de engenheiro — o produto é só orçamento. A calibração contra obra com projeto estrutural real segue como lacuna de DADO (família da R6): quando existir, entra como oráculo no mesmo teste.
